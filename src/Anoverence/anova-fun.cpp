...
selectPairs()
{
  ...
  for (auto& itr : experimentMap) { // for each pair
    for (auto& k : itr.second) {
      ...
      for (auto& c :
           itr.second) { // TODO maybe we can top the iteration until k index
      ...
      for (int j = 0; j < 2; j++) { // for each gene
        ...
        for (int i = 0; i < repeats; ++i) {
          ...
          Pijk = (j == 1 && isPerturbed(genePair[j], &chips[k]) &&
                  !isPerturbed(genePair[j], control))
                   ? wko
                   : 1;
                    ...
                    float wko)
{
  ...
  for (int j = 0; j < geneNumber; ++j) {
    genePair[0] = j;
    for (auto t : targets) {
      auto pos = std::distance(geneNames.begin(),
                               find(geneNames.begin(), geneNames.end(), t));
      genePair[1] = pos;
...
totalPerturbed(chip* c1, chip* c2)
{
  ...
  return countPerturbed(c1->perturbationsID, c2->perturbationsID) +
...
isPerturbationEqualSet(chip* c1, chip* c2)
{
  ...
  return (c2->perturbationsID == c1->perturbationsID) &&
...
compareChips(chip c1, chip c2)
{
  ...
  if (c1.deletedGenes.size() < c2.deletedGenes.size())
    ...
    if (c1.deletedGenes.size() != 0) {
      for (anovaIndex i = 0; i < c1.deletedGenes.size();
           i++) { // compare each pair
  ...
  log(LOG_WARN) << "Chip " << c1.chipNumber << " and chip " << c2.chipNumber
                << " are repeated.\n";
  return false;
}
