...
knot_vector(size_t spline_order, size_t num_bins)
{
  ...
  for (size_t i = spline_order + int_points; i < 2 * spline_order + int_points;
       i++) {
    ret[i] = 1;
  }
       ...
       size_t num_bins)
{
  ...
  for (size_t cur_bin_x = 0; cur_bin_x < num_bins; cur_bin_x++) {
    for (size_t cur_bin_y = 0; cur_bin_y < num_bins; cur_bin_y++) {
      for (size_t cur_sample = 0; cur_sample < x.n_elem; cur_sample++) {
        double n = x.n_elem;
        hist(cur_bin_x, cur_bin_y) += wx(cur_bin_x * x.n_elem + cur_sample) *
