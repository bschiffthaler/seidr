...
void
print_centrality(const seidr_param_view_t& param,
...
void
finish_binary(const seidr_param_view_t& param, SeidrFileHeader& hh)
{
...
bool
max_lines_reached(const po::variables_map& vm,
...
bool
print_and_check_maxl(const SeidrFileEdge& e,
                     const SeidrFileHeader& h,
                     std::ostream& out,
                     const std::vector<std::string>& pf,
                     const seidr_param_view_t& param,
...
void
print_titles(const seidr_param_view_t& param,
             const SeidrFileHeader& h,
             std::ostream& out)
{
...
view(const std::vector<std::string>& args)
{
  ...
  try {
    ...
    assert_mutually_exclusive(vm,
                              { "binary", "header", "centrality", "dense" });
