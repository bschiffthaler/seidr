...
std::vector<std::string> get_targets(const std::string& p)
{
...
double
diff_score(const seidr_dna_param_t& param,
...
int dna(int argc, char * argv[]) {
  ...
  ("n-permutations,B",
   ...
   "The number of permutations to use when calculating the differential score"),
  ("target,t",
   po::value<std::string>(&param.target),
   "Calculate P-value for these pathways (can be multiple comma separated or a "
   "file with one target per line)");

  po::options_description req("Required Options [can be positional]");
  req.add_options()
  ("gene-to-pathway", po::value<std::string>(&param.gene_to_pw)->required(),
  ...
  arma::mat X1 = read_network_arma(h1, n1,
                                   -std::numeric_limits<double>::infinity(),
                                   param.tindex1, false);
  log(LOG_INFO) << "Reading network 2 into memory\n";
  arma::mat X2 = read_network_arma(h2, n2,
                                   -std::numeric_limits<double>::infinity(),
                                   param.tindex2, false);

  std::vector<std::string> sorted_isect = gene_intersect(h1.nodes(), h2.nodes());
