...
void SeidrFileHeader::print_centrality(std::ostream& out) const
{
  uint32_t nmeasures = attr.pagerank_calc + attr.closeness_calc +
                       ...
                       attr.katz_calc;
  if ( nmeasures == 0)
    throw std::runtime_error("At least one node centrality measure must be calculated\n");
  for (uint32_t i = 0; i < attr.nodes; i++)
  {
    auto nm = nmeasures;
    (out) << nodes[i] << '\t';
...
std::set<offset_t>
SeidrFileIndex::get_offset_nodelist(std::vector<std::string>& nodelist)
{
...
bool me_score_sort_abs(MiniEdge a, MiniEdge b) { return fabs(a.s) > fabs(b.s); }

bool me_score_sort_rev(MiniEdge a, MiniEdge b) { return (a.s < b.s);}

bool sfe_score_sort(SeidrFileEdge a, SeidrFileEdge b) {
  return (a.scores[a.scores.size() - 1].s >
          b.scores[b.scores.size() - 1].s);
}
