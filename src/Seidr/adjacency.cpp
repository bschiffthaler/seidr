...
void
make_full(const seidr_param_adjacency_t& param)
{
...
void
make_lower(const seidr_param_adjacency_t& param)
{
  SeidrFile sf(param.el_file.c_str());
  sf.open("r");
  SeidrFileHeader h;
...
adjacency(const std::vector<std::string>& args) // NOLINT
{
  ...
  try {
    ...
    fopt.add_options()("diagonal,D",
                       po::bool_switch(&param.diag)->default_value(false),
                       "Print matrix diagonal for triangular output format")(
      "fmt,F",
      po::value<std::string>(&param.outfmt)->default_value("m"),
      "Output format ['m','lm']")(
      "missing,m",
      po::value<std::string>(&param.fill)->default_value("0"),
      "Fill character for missing edges")(
      "precision,p",
      po::value<uint16_t>(&param.prec)->default_value(ADJACENCY_DEF_PRECISION),
