...
std::unique_ptr<ranger::Data>
SeidrForest::load(const std::vector<std::string>& dependent_variable_name,
                  const arma::mat& gm_x,
                  const arma::mat& gm_y,
                  const std::vector<std::string>& var)
{
  ...
  std::ostream& ostream)
{
  ...
  init(M,
       load(dependent_variable_name, gm_x, gm_y, var),
       mtry,
            ...
            const seidr_genie3_param_t& param)
{
  ...
  seidr_mpi_genie3 mpi(param.bs, gm, uvec, genes, param.tempdir, param.outfile);
