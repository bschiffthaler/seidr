  ...
  _queue_fh(nullptr)
{
  ...
  {
    MPI_File ftmp;
    MPI_File_open(MPI_COMM_SELF, // NOLINT(cppcoreguidelines-pro-type-cstyle-cast)
                  _queue_file.c_str(),
                  MPI_MODE_CREATE | MPI_MODE_WRONLY, // NOLINT
                  MPI_INFO_NULL, &ftmp); // NOLINT(cppcoreguidelines-pro-type-cstyle-cast)
    MPI_Status status;
    MPI_File_write(ftmp, &_indices[0], _indices.size(), MPI_UINT64_T, &status); // NOLINT(cppcoreguidelines-pro-type-cstyle-cast)
    MPI_File_close(&ftmp);
  }
  // Everyone needs to wait until queue is set up
  MPI_Barrier(MPI_COMM_WORLD); // NOLINT(cppcoreguidelines-pro-type-cstyle-cast)

  MPI_File_open(MPI_COMM_WORLD, // NOLINT(cppcoreguidelines-pro-type-cstyle-cast)
                _queue_file.c_str(),
                MPI_MODE_RDONLY, // NOLINT(cppcoreguidelines-pro-type-cstyle-cast)
                MPI_INFO_NULL, &_queue_fh); // NOLINT(cppcoreguidelines-pro-type-cstyle-cast)
  get_more_work();
}
...
void seidr_mpi_omp::get_more_work()
{
  ...
  MPI_File_read_shared(_queue_fh, &_my_indices[0],
                       _bsize, MPI_UINT64_T, &status); // NOLINT(cppcoreguidelines-pro-type-cstyle-cast)

  int work;
  MPI_Get_count(&status, MPI_UINT64_T, &work); // NOLINT(cppcoreguidelines-pro-type-cstyle-cast)
...
{
  std::string host;
  host.resize(MPI_MAX_PROCESSOR_NAME); // NOLINT(cppcoreguidelines-pro-type-cstyle-cast)
  int hostlen;
