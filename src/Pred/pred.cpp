...
main(int argc, char** argv)
{
  ...
  try {
    ...
    TCLAP::ValueArg<std::string> infile_arg(
      "i", "infile", "The data file", true, "", "string");
    cmd.add(infile_arg);

    TCLAP::ValueArg<std::string> model_arg(
      "m", "model", "File containing ML model", true, "", "string");
    cmd.add(model_arg);

    TCLAP::ValueArg<std::string> outfile_arg(
      "o", "outfile", "Output file path", false, "", "string");
    cmd.add(outfile_arg);

    // keeping it as a string for future uses of other models
    TCLAP::ValueArg<std::string> classifier_arg(
      "c",
      "classType",
  ...
  try {
    ...
    log(LOG_ERR) << e.what() << '\n';
    return 1;
  } catch (const std::exception& e) { // TODO treat other exceptions differently
    log(LOG_ERR) << e.what() << '\n';
    return 1;
  }
  return 0;
