         ...
         double _alpha, arma::vec _ulam)
{
  ...
  double * wptr = new double[nobs]();
  for(int i = 0; i < nobs; i++)
    wptr[i] = 1.0;

  double * vpptr = new double[nvars]();
  for(int i = 0; i < nvars; i++)
    vpptr[i] = 1.0;

  double * clptr = new double[2 * nvars]();
  ...
  if(flmin >= 1)
    {
      ...
      try
        {
          nlam = boost::numeric_cast<int>(_ulam.n_elem);
        }
  ...
  double * captr = new double[nx * nlam]();
  int * iaptr = new int[nx]();
  int * ninptr = new int[nlam]();
  double * rsqptr = new double[nlam]();
  double * almptr = new double[nlam]();
