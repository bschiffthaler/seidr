...
int main(int argc, char ** argv) {
  ...
  try
  {
    ...
    targets_arg("t", "targets", "File containing gene names of genes of interest."
                ...
                " the sources of potential connections.", false, "", "");
    cmd.add(targets_arg);

    TCLAP::ValueArg<unsigned long>
    batchsize_arg("b", "batch-size", "Number of genes in MPI batch",
                  false, 20, "20");
    ...
    max_pred_arg("P", "max-predictor-size", "The maximum number of "
                 "preditors to be sampled", false, 0, "4/5th of genes");
    cmd.add(max_pred_arg);

    TCLAP::ValueArg<seidr_uword_t>
    min_exp_arg("x", "min-experiment-size", "The minimum number of "
                "experiments to be sampled.", false, 0, "1/5th of experiments");
    cmd.add(min_exp_arg);
  ...
  if (rank == 0)
  {
    try
    {
      ...
      std::string tempdir = dirname(outfile) + "/.seidr_tmp_svm";
      ...
      if (! create_directory(dirname(outfile), ".seidr_tmp_svm") )
